...
def test_list_workflows_viewer_blocked(client, engine):
    ...
    resp = client.get("/automation/workflows", headers=auth_header(token))
...
def test_list_workflows_filter_by_agent_id(client, engine):
    ...
    resp = client.get("/automation/workflows?agent_id=agent-x", headers=auth_header(token))
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) == 1
...
def test_list_workflows_filter_by_status(client, engine):
    ...
    resp = client.get("/automation/workflows?status=failed", headers=auth_header(token))
    assert resp.status_code == 200
    data = resp.json()
    assert len(data) == 1
...
def test_get_workflow_by_id(client, engine):
    ...
    resp = client.get(f"/automation/workflows/{wf.id}", headers=auth_header(token))
    assert resp.status_code == 200
...
def test_get_workflow_not_found(client, engine):
    ...
    resp = client.get("/automation/workflows/99999", headers=auth_header(token))
...
def test_reload_requires_admin(client, engine):
    ...
    resp = client.post("/automation/reload", headers=auth_header(token))
    assert resp.status_code == 403
