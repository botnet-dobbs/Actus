...
def test_list_users_requires_admin(client, engine):
    ...
    resp = client.get("/auth/users", headers=auth_header(token))
...
def test_list_users_pagination(client, engine):
    ...
    resp = client.get("/auth/users?limit=2&offset=0", headers=auth_header(token))
    assert resp.status_code == 200
...
def test_delete_user_requires_admin(client, engine):
    ...
    resp = client.delete(f"/auth/users/{target.id}", headers=auth_header(token))
    assert resp.status_code == 403
...
def test_delete_user_soft_deletes(client, engine):
    ...
    resp = client.post("/auth/login", data={"username": "target2", "password": "testpass"})
...
def test_delete_user_not_found(client, engine):
    ...
    resp = client.delete("/auth/users/99999", headers=auth_header(token))
...
def test_admin_cannot_delete_self(client, engine):
    ...
    resp = client.delete(f"/auth/users/{admin.id}", headers=auth_header(token))
...
def test_password_reset_allows_new_login(client, engine):
    ...
    resp = client.post("/auth/login", data={"username": "target4", "password": "testpass"})
    assert resp.status_code == 401
    # New password works
    resp = client.post("/auth/login", data={"username": "target4", "password": "newpassword123"})
    assert resp.status_code == 200
